package com.example.homeworks.utills
...
fun String.eval(): Double {
    ...
    return object : Any() {
        ...
        var ch = 0
        fun nextChar() {
            ch = if (++pos < this@eval.length) this@eval[pos].toInt() else -1
        }
        ...
        fun parse(): Double {
            ...
            if (pos < this@eval.length) throw RuntimeException("Unexpected: " + ch.toChar())
            return x
        }
        ...
        fun parseFactor(): Double {
            ...
            if (eat('('.toInt())) { // parentheses
                ...
                x = this@eval.substring(startPos, pos).toDouble()
            } else if (ch >= 'a'.toInt() && ch <= 'z'.toInt()) { // functions
                while (ch >= 'a'.toInt() && ch <= 'z'.toInt()) nextChar()
                val func = this@eval.substring(startPos, pos)
                x = parseFactor()
                x =
